...
func (h *EducationHandler) handleViewEducation(w http.ResponseWriter, r *http.Request) {
	...
	educations, err := h.educationStore.GetEducations(userID, limit, offset)
	if err != nil {
		httputil.WriteError(w, http.StatusInternalServerError, err)
		return
	}

	total, err := h.educationStore.CountByUserID(userID)
	if err != nil {
		httputil.WriteError(w, http.StatusInternalServerError, err)
		return
	}

	resp := education.PaginatedEducationsResponse{
		Data: educations,
	}
...
func (h *EducationHandler) handleDeleteEducation(w http.ResponseWriter, r *http.Request) {
	...
	userID := middleware.GetUserIDFromContext(r.Context())
	educ, err := h.educationStore.DeleteEducation(id, userID)
	if err != nil {
		httputil.WriteError(w, http.StatusInternalServerError, err)
		return
	}
